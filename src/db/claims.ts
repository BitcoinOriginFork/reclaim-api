...
export async function createClaim (btcoAddress: string, signature: string): Promise<Claim> {
  await queryHandler(async function (client: Client) {
    await client.query(
      `INSERT INTO claims ("btcoAddress", signature, "status", "createdAt", "updatedAt") VALUES ($1, $2, $3, $4, $5)`,
      [btcoAddress, signature, ClaimStatus.pending, new Date(), new Date()]
    )
  })
